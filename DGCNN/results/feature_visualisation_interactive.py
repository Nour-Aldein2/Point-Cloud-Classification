...
```
python -m DGCNN.results.feature_visualisation_interactive --ckpt DGCNN/checkpoints/best_model_512_10.pt --split test --method tsne
```
...
directory rather than opening the file through `file://`, for example
```
...
def _make_loader(dataset, cfg: Config):
    return DataLoader(
        dataset=dataset,
...
def prepare_data(cfg: Config, split_name: str, render_num_points: int):
    model_dataset = PointCloudData(root_dir=cfg.data.root_dir,
                                   num_points=cfg.data.num_points,
                                   split_name=split_name,
                                   seed=cfg.data.seed)

    render_num_points = cfg.data.num_points if render_num_points <= 0 else render_num_points
    if render_num_points == cfg.data.num_points:
        render_dataset = model_dataset
    else:
        render_dataset = PointCloudData(root_dir=cfg.data.root_dir,
    ...
    model_loader = _make_loader(model_dataset, cfg)
    render_loader = _make_loader(render_dataset, cfg)

    return model_loader, render_loader, model_dataset.classes


def _get_final_classifier(model):
    linear_layers = [(name, module) for name, module in model.named_modules()
                     if isinstance(module, torch.nn.Linear)]

    if not linear_layers:
        raise ValueError("Could not find a Linear classifier layer in the DGCNN model.")
...
def extract_embeddings(model, loader, device):
    ...
    classifier_name, classifier = _get_final_classifier(model)
    print(f"Extracting embeddings from the input to final classifier: {classifier_name}")
    ...
    hook = classifier.register_forward_pre_hook(save_embedding)
...
def extract_render_clouds(loader):
    ...
    for batch in tqdm(loader, desc="Loading 3D shapes", ncols=80, leave=False, colour="#73b0c9"):
        batch_clouds = []

        for graph_idx in range(batch.num_graphs):
            start = int(batch.ptr[graph_idx])
            end = int(batch.ptr[graph_idx + 1])
            cloud = batch.pos[start:end]

            if cloud.ndim != 2 or cloud.shape[1] < 3:
                raise ValueError(
                    "Expected each point cloud to contain XYZ coordinates, "
                    f"but received shape {tuple(cloud.shape)}."
                )

            batch_clouds.append(cloud[:, :3].detach().cpu())

        point_counts = {len(cloud) for cloud in batch_clouds}
        if len(point_counts) != 1:
            raise ValueError(
                "All shapes in a rendering batch must contain the same number of points. "
                f"Received counts {sorted(point_counts)}."
            )

        point_clouds.append(torch.stack(batch_clouds, dim=0))
        labels.append(batch.y.view(-1).detach().cpu())

    if not point_clouds:
        raise ValueError("No point clouds found for feature visualisation.")

    return torch.cat(point_clouds, dim=0).numpy(), torch.cat(labels, dim=0).numpy()
...
def make_feature_plot(
    ...
    title = f"Learned Feature Space ({method_name}) — {experiment_name}"
    payload = {
        "title": title,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualise learned DGCNN embeddings.")
    ...
    parser.add_argument("--points-per-shape", type=int, default=2048,
                        help="Number of points rendered in the selected-shape preview.")
    parser.add_argument("--max-shapes", type=int, default=0,
                        help="Maximum scatter samples embedded in the page; 0 keeps all samples.")
    parser.add_argument("--marker-size", type=float, default=14.0,
                        help="Initial scatter-marker diameter in pixels (range: 8-25).")
    parser.add_argument("--preview-point-size", type=float, default=0.05,
                        help="Initial point size in the selected-shape preview (range: 0.01-0.10).")
    parser.add_argument("--axis-order", "--axis-map", dest="axis_order", default="x,z,-y",
                        help="Signed source-to-rendered axis mapping.")
    parser.add_argument("--three-version", default=THREE_VERSION,
                        help="Pinned Three.js version loaded by the generated webpage.")
    ...
    model_loader, render_loader, classes = prepare_data(cfg,
                                                         split_name,
                                                         render_num_points=args.points_per_shape)

    model = model.to(cfg.train.device)

    embeddings, labels = extract_embeddings(model, model_loader, cfg.train.device)
    ...
    try:
        node_count = checkpoint_path.name.split("_")[2]
        experiment_name = f"DGCNN {node_count} Nodes"
    except IndexError:
        experiment_name = "DGCNN"

    output_path = args.output
    if output_path is None:
        checkpoint_name = checkpoint_path.stem.removeprefix("best_model_")
        output_path = Path("./Figures") / f"feature_space_{method}_{checkpoint_name}.html"
